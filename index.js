...
exports.plugin = (schema, options) => {
  ...
  IdentityCounter.findOne({ model: settings.model, field: settings.field }, (err, counter) => {
    if (!counter) {
      ...
      counter = new IdentityCounter({ // eslint-disable-line
  ...
  const nextCount = (callback) => {
    IdentityCounter.findOne({
      ...
      const count = counter === null ? settings.startAt : counter.count + settings.incrementBy;
  ...
  const resetCount = (no, callback) => {
    IdentityCounter.findOneAndUpdate(
      { model: settings.model, field: settings.field },
      { count: no || settings.startAt - settings.incrementBy },
  ...
  schema.pre('save', function savingSchema(next) {
    ...
    if (doc.isNew) {
      ...
      (function save() { // eslint-disable-line
        ...
        // Note: ready is true when an existing counter collection is found
        // or after it is created for the first time.
        if (ready) {
          // check that a number has already been provided, and update the counter to
          // that number if it is greater than the current count
          if (typeof doc[settings.field] === 'number') {
            IdentityCounter.findOneAndUpdate(
              // IdentityCounter documents are identified by the model and field that the
              // plugin was invoked for. Check also that count is less than field value.
            ...
            IdentityCounter.findOneAndUpdate(
              ...
              // field that the plugin was invoked for.
              { model: settings.model, field: settings.field },
              ...
              (err, updatedIdentityCounter) => {
                if (err) return next(err);
                // If there are no errors then go ahead and set the
                // document's field to the current count.
          ...
          // If not ready then set a 5 millisecond timer and try to save again. It will keep
          // doing this until the counter collection is ready.
          setTimeout(save, 5);
        }
      })();
    } else {
      // If the document does not have the field we're interested in or that field isn't a number
      // AND the user did not specify that we should increment on updates, then just continue
      // the save without any increment logic.
      next();
    }
  });
};
